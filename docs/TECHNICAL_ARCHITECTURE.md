...
Run upgrades are configured in `src/config/upgrades.ts` and owned by `UpgradeSystem`. The shop is a `shop` run state with a DOM/CSS overlay, entered after stage clear only when at least one non-maxed upgrade is affordable. Buying checks currency, subtracts cost, increases the level, updates HUD/shop currency immediately, and leaves gameplay paused until Skip / Continue. Fire rate feeds the existing projectile cooldown path and clamps against the configured normal and rapid-fire minimums. Projectile damage is passed into combat so tank enemies require fewer upgraded hits. Projectile speed is passed through the existing projectile pool update path. Shield duration is added when future shield pickups are applied. Score bonus multiplies score rewards only, not currency. Restart and Main Menu reset all upgrade levels; continuing from the shop preserves lives, score, currency, and upgrades.

Menu overlays are DOM/CSS modules under `src/ui`: Main Menu, Pause Menu, Shop, Game Over, and Stage Clear. `GameApp` owns a small run state union: `mainMenu`, `playing`, `paused`, `stageClear`, `shop`, `gameOver`, and `runComplete`. Gameplay updates are state-gated so spawning, shooting, movement, combat, hit bursts, and stage timing run only in `playing`. Escape toggles pause only from active gameplay; it does not pause from Main Menu, Shop, Game Over, Stage Clear, or Run Complete.
